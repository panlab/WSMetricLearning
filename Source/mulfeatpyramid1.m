function pyra = mulfeatpyramid1(im, model,imageid, padx, pady)
...
for i = 1:interval
  ...
  % "first" 2x interval
  pyra.feat{i} = mulfeatures1(scaled, model, sbin/2, 0.5, imageid,[],[]);
  pyra.scales(i) = 2/sc^(i-1);
  % "second" 2x interval
  pyra.feat{i+interval} = mulfeatures1(scaled, model, sbin,1, imageid,[],[]);
  pyra.scales(i+interval) = 1/sc^(i-1);
  % remaining interals
  for j = i+interval:interval:max_scale
    scaled = resize(scaled, 0.5);
    pyra.feat{j+interval} = mulfeatures1(scaled, model, sbin, 1 ,imageid,[],[]);
