function [dPsi, M, SO_time, hLatent, RLatent] = cuttingPlaneFull_Latent_f3(k, X, hLatent, RLatent, ...
...
%     S1      = zeros(n);
    ...
    if isempty(ClassScores)
        ...
        TSAMPLES = [1:ntest*Nrotate];
            ...
            Ntest= length(TSAMPLES);
            
D2 = D1;
            tic
            D1 = -D1; 
            MRR  = ((1:(nbase)).^-1)';
            Ypos1 = cell2mat(Ypos(C_index));
            ind1 = sub2ind(size(D1), Ypos1, [1:Ntest]');          
            Dpos = D1(ind1);
            D1(ind1) = inf;
            [Dneg,ord] = sort(D1, 'descend');ord = ord(2:end,:);
            Dneg(1,:) = Dpos';
            Td = repmat(Dpos',[nbase,1]) - Dneg;
            std = cumsum(Td) /(nbase-1); 
            I = -2*std-repmat(MRR, [1,Ntest]);
            [I1, id] = max(I);
            Index = sub2ind(size(yik), id, [1:Ntest]);
            yik(Index) = Ypos1';
            yik(setdiff([1:Ntest*nbase], Index)) = ord(:);
            lik = (1 - MRR(id))';
            sik = std(end,:) + I1+1;
            toc
...
%             Yneg1 = Yneg(C_index);
%             Ypos1 = Ypos(C_index);
%             yik1 = yik;lik1 = lik;sik1 = sik;
%             parfor j = TSAMPLES
%                 D = D2(:,j);
%                 [yik(:,j), lik(j), sik(j)]   =   separationOracleMRR(1, D,...
%                     Ypos1{j}, Yneg1{j}, k);
%             end
%             
%             max(abs(yik1 - yik))
...
%             max(abs(sik1 - sik))
            ...
            tic
             for j = SAMPLES'
                ...
                TS(j-ntrain,:)         = PSIALL(j, yi'+plus(RLatent(j)), n, Ypos{j}+...
