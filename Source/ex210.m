...
%   x  is  'K' for K-SVD, 'I' for ILS-DLA (MOD java implementation),  
...
elseif (strcmpi(metChar,'M'))   
    ...
    % RLS-DLA with different approaches for increasing the forgetting factor
...
for exno = 1:nofTrials
    ...
    %
    Dorg = dictmake(N, K, 'G');
    X = datamake(Dorg, L, s, snr, 'G');
    % sumXX = sum(sum(X.*X));
    D = dictnormalize( X(:,floor(0.85*L-K)+(1:K)) ); %
    jD0 = mpv2.SimpleMatrix(D);
    %
    tic;
    if strcmpi(metChar,'M') 
        for it = 1:noIt
