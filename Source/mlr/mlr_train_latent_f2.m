function [W, Xi, Diagnostics, Wtmp] = mlr_train_latent_f2(W, X, Y, numSign, ...
...
%     rotate = [0, 10, -10];
    ...
    [d,n,m] = size(getLatent(numSign, X));
...
%         hLatent1 = hLatent;
        if num_calls_latent >1
            PsiLatent1 =  Compute(k, X, hLatent, RLatent, numSign, W, ...
        ...
        [PsiLatent, hLatent, RLatent]        = LATENT(k, X, hLatent, RLatent, numSign, W, ...
            Ypos, Yneg, batchSize, Template, Nrotate, SAMPLES, ClassScores);
        if num_calls_latent >1
            dp= sum(sum(W.*(PsiLatent1 - PsiLatent)));
            if dp > 0
                t = 1;
            end
        end
        ...
        while Diagnostics.num_calls_solver < MAXITER
            ...
            H           = expandKernel(H);
            Q           = expandRegularizer(Q, getLatent(numSign, X, hLatent), W);
...
%             th = tic;
            ...
            PsiClock                        = PsiClock + 1;
            Solver_time                     = tic();
            [W, Xi, Dsolver]                = mlr_admm(C, getLatent(numSign, X, hLatent), Margins, H, Q);
...
%         Diagnostics.f
        ...
        if num_calls_latent > 0
            ...
            if (abs((f_old - Diagnostics.f(end)) / f_old) < E)
                ...
                ObjDP{num_calls_latent} = dp;
                Wtmp{num_calls_latent} = W;
%                 FF = Diagnostics.f;Nc = Diagnostics.num_constraints(end);
%                 save(['ObjFun_',num2str(num_calls_latent)], 'FF', 'Nc')
...
%         save(['ObjFun_',num2str(num_calls_latent)], 'FF', 'Nc')
    ...
    Diagnostics.ObjFun = ObjFun;Diagnostics.ObjCons = ObjCons;Diagnostics.ObjDP = ObjDP;
end
