...
double Edsit(double A, double B){
  double dist = (A - B) * (A - B);
  return dist;
}
...
void *process(void *thread_arg) {
  ...
  for (int f = 0; f < num_features; f++) {
	...
	for (int xp = 0; xp < B_dims[1]; xp++) {
	  ...
	  switch(B_dims[0]) {
	  case 20: val += Edsit(A_off[19] , B_off[19]);
	  case 19: val += Edsit(A_off[18] , B_off[18]);
	  case 18: val += Edsit(A_off[17] , B_off[17]);
	  case 17: val += Edsit(A_off[16] , B_off[16]);
	  case 16: val += Edsit(A_off[15] , B_off[15]);
	  case 15: val += Edsit(A_off[14] , B_off[14]);
	  case 14: val += Edsit(A_off[13] , B_off[13]);
	  case 13: val += Edsit(A_off[12] , B_off[12]);
	  case 12: val += Edsit(A_off[11] , B_off[11]);
	  case 11: val += Edsit(A_off[10] , B_off[10]);
	  case 10: val += Edsit(A_off[9] , B_off[9]);
	  case 9: val += Edsit(A_off[8] , B_off[8]);
	  case 8: val += Edsit(A_off[7] , B_off[7]);
	  case 7: val += Edsit(A_off[6] , B_off[6]);
	  case 6: val += Edsit(A_off[5] , B_off[5]);
	  case 5: val += Edsit(A_off[4] , B_off[4]);
	  case 4: val += Edsit(A_off[3] , B_off[3]);
	  case 3: val += Edsit(A_off[2] , B_off[2]);
	  case 2: val += Edsit(A_off[1] , B_off[1]);
	  case 1: val += Edsit(A_off[0] , B_off[0]);
	    break;
	  default:	    	      
	    for (int yp = 0; yp < B_dims[0]; yp++) {
	      val += Edsit(*(A_off++) , *(B_off++));
	    }
	  }
	}
	*(dst++) += val;
