function [dataX, dataY, inum, setting, Label, dataXX] = getNewtemplate(idx1, idx2,  dataX, dataY,...
    ts_label,  setting,  SW)   
% addpath('ksvdbox');addpath('ompbox')
if ~isfield(setting, 'Nfixed')
    setting.Nfixed  = 0;
end
dataXX = [];Label = setting.Label;
    ...
    if setting.TemplateINNC
        ...
        for ii = 1:length(a)
            ...
            if setting.MeanNum == 1
                ...
                [xmeans, label] = Wkmeans_W(dataX(idx, :), ...
                    setting.MeanNum, SWW, setting.Svlfeat, setting.innerkmean);
            end
%             dis = sum(dataX.^2,2) - 1;max(abs(dis(:)))
%             dis = sum(xmeans.^2,1);(abs(dis))
...
switch TemplateINNC
    case 1
        setting.MetricL2 = setting.isMetricL2;
        ...
        if setting.Nfixed
            ...
            CodeINN  = INNCode(dataT, [1:size(dataT, 1)]', dataX,  ...
                cellfun(@max, dataY(inum+1:end, 1), 'UniformOutput', true), ...
                setting.lambda_INNC, setting.K_INNC, setting.blocksize_INNC, ...
                setting.verbose_INNC, setting.beta_INNC, Ymatched, setting.innerfea);
    ...
    case 2
        ...
        SWW = [ones(inum, 1);SWW];
        if setting.K_dic == 1
            Data = bsxfun(@times, dataX', SWW');
            D = mean(Data, 2);
        else
            D = Wkmeans_W(dataX, setting.K_dic, SWW, setting.Svlfeat);
        end
%         if setting.TemplateNorm
%             D = L2normMatrix(D);
%         end
...
if ~params.fixedcode
    ...
    if ~strcmp(setting.method_dic, 'SLEP')
        params.method_dic = 'L0';%????
    else
