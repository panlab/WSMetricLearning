function [D,Gamma,err,gerr] = ksvd1(params,varargin)
...
if isfield(params, 'fixedcode') && params.fixedcode
    Gamma = sparse(params.W);
end
...
for iter = 1:iternum
  ...
  %%%%%  sparse coding  %%%%%
  if ~params.fixedcode
      Gamma = sparsecode(data,D,XtX,G,thresh,params.method_dic,params.lamda,params.SCode);
      if ~isempty(params.SCode.Code)
  ...
  if (testgen)
    ...
    GammaG = sparsecode(testdata,D,XtXg,G,thresh, params.SCode);
...
function Gamma = sparsecode(data,D,XtX,G,thresh, method,lamda, SCode)
switch method
    case 'L0'
...
% % %         Gamma=l1_ls(D,data,lamda,rel_tol);
        root=cd;
        addpath(genpath([root '/SLEP']));
        cd SLEP/Examples/L1;
        opts=[];
        opts.rsL2=0;        % the two norm regularization
        ...
        for xx = 1:size(data, 2)
            [Gamma(:,xx), b]= LeastC(D, (data(:,xx)), lamda, opts);
        end
        cd ..
        cd ..
        cd ..
    ...
    case 'Own'
        rel_tol = 0.01;     % relative target duality gap
        NK = size(D, 2);
        NT = SCode.nTemp;NQ = size(data, 2) - NT;ND = size(data, 1);
        Gamma = SCode.fun(NK, SCode.C, D, data(:, 1:SCode.nTemp),lamda,[]);
        VT = diag(Gamma'*SCode.W*Gamma);
        SCode.P1 = SCode.P1*Gamma'*SCode.W;
        SCode.Q1 = SCode.Q0 + SCode.Q1 * VT;
        ncons = length(SCode.Q1);
        SCode.P1 = (reshape((SCode.P1)', [], ncons))';
        Gamma(:, SCode.nTemp+1:(NQ+NT))=SCode.fun(NK, SCode.C, D, data(:, SCode.nTemp+1:(NQ+NT)),...
            SCode.lamda_dic,SCode);
        Gamma = sparse(Gamma);
    ...
    case 'SLEP'
        ...
        Gamma = SCode.Code;
        NP = size(Gamma, 2);ND = size(Gamma, 1);
%         SCode.lamda_dic = 1 /NP* SCode.lamda_dic;
        OBJ = Inf;
        Range1 = [1:SCode.nTemp];Range2 = [SCode.nTemp+1:NP];
        invD = pinv(D);
        Q1 = SCode.Q0;  %%%0.724
        Q1  = Q1(:);
        cRange = {Range1, Range2};
        b = zeros(1, max(Range2));
        for i = 1:2*SCode.iter_dic
            tic;
            if i==1 || ~mod(i,2)
                PO1 = SCode.P1*Gamma(:, Range1)'*SCode.W;   %%%4591*100
            end
            TT = sum(-4*PO1 .* repmat(Gamma(:, Range2)', SCode.ncons, 1), 2); %%% 4591*1
            TT = bsxfun(@plus, Q1', sum((reshape(TT, [], SCode.ncons)), 1));%%%  0.6557
            [a,idx] = max([TT, 0], [], 2);
            OBJ1 = 1 /NP * (sum(sum((data - D*Gamma).^2))) + SCode.C * a;
            if mod(i, 2) && abs(OBJ1 - OBJ) / abs(OBJ) < 1e-3
                break;
            end
            OBJ = OBJ1;
            Temp = [PO1; zeros([NP-SCode.nTemp,ND])];  %%% 9182*100
            Range = [(idx-1)*(NP-SCode.nTemp)+1:idx*(NP-SCode.nTemp)];
            P1 = Temp(Range, :);   %%%4591*100
            Temp = [SCode.P1; zeros([NP-SCode.nTemp,SCode.nTemp])];
            if i==1 || mod(i,2)
                DT = -4*(Gamma(:, Range2)*Temp(Range, :))' * SCode.W;   %%%62*100
            end
            
            y = data' - NP/2*SCode.C*[DT', -4*P1']'*invD;
            if mod(i,2)==0
                idx= cRange{2-mod(i,2)};
          
                th1 = tic;
                for xx = idx
                    [Gamma(:,xx), B]= LeastC(D, (y(xx,:))', SCode.lamda_dic(xx), opts);
                    b(xx) = B(end);
                end
                th1 = toc(th1);
                Gamma1 = Gamma; b1 = b;
                th2 = tic;
                [Gamma(:,idx), B]= LeastC_m(D, (y(idx,:))', SCode.lamda_dic(idx), opts);
                b(idx) = (B(end,:))';
                th2 = toc(th2);
                dis1 = Gamma1 - Gamma;
                dis = b1 - b;max(abs(dis(:)));
                [max(abs(dis1(:))), max(abs(dis(:))), th1 / th2]
            end
...
% 	    toc;
    ...
    case 'SLEP_L0'
        ...
        for i = 1:2*SCode.iter_dic
            ...
            y = data' - NP/2*SCode.C*[DT', -4*P1']'*invD;
            OBJ2 = 0;
            OBJ2 = OBJ2 + 1 /NP *(sum(sum((data(:, Range1) - D*Gamma(:, Range1)).^2)));
            OBJ2 = OBJ2+1 /NP * (sum(sum(((y(Range2,:))' - D*Gamma(:, Range2)).^2))) + ...
                1 /NP * (sum(sum((data(:, Range2).^2))))- 1/NP * (sum(sum(((y(Range2,:)).^2))));
            OBJ2 = OBJ2 + SCode.C * [Q1; 0];
            dis = OBJ1 -OBJ2(idx);
            max(abs(dis(:)))
            idx = cRange{2-mod(i,2)};
            Gamma(:,idx) = sparsecode((y(idx,:))',D,XtX,G,thresh, 'L0', SCode.lamda_dic(idx), SCode);
            Gamma(:,idx) = (L2normMatrix((Gamma(:,idx))', 1))';
%             sum(Gamma(:,idx).^2, 1)
            toc;
        end
        Gamma = sparse(Gamma);
 case 'LGrad'
        ...
        if SCode.C == 0
            ...
            OBJ1 = 1 /NP * (sum(sum((data - D*Gamma).^2)) + sum(sum((bsxfun(@times, abs(Gamma), ...
                SCode.lamda_dic)))));
            if abs(OBJ1 - OBJ) / abs(OBJ) < 1e-3
                break;
            end
            dC =  1 /NP * (dcc + 2 * D' * (D * Gamma) + sign(Gamma)*SCode.lamda_dic);
            Gamma  = Gamma  - SCode.mult_dic * 1 / sqrt(i) * dC;
            ...
            for i = 1:SCode.iter_dic
            tic;
            P1 = SCode.P1*Gamma(:, Range1)'*SCode.W;   %%%4591*100
            Q1 = SCode.Q0;  %%%0.724
            Q1  = Q1(:);
            TT = sum(-4*P1 .* repmat(Gamma(:, Range2)', SCode.ncons, 1), 2); %%% 4591*1
            TT = bsxfun(@plus, Q1', sum((reshape(TT, [], SCode.ncons)), 1));%%%  0.6557
            [a,idx] = max([TT, 0], [], 2);
            OBJ1 = 1 /NP * (sum(sum((data - D*Gamma).^2)) + sum(sum((bsxfun(@times, abs(Gamma), ...
                SCode.lamda_dic))))) + SCode.C * a;
            if abs(OBJ1 - OBJ) / abs(OBJ) < 1e-3
                break;
            end
            LL(i) = OBJ1; 
            OBJ = OBJ1;
            Temp = [P1; zeros([NP-SCode.nTemp,ND])];  %%% 9182*100
            Range = [(idx-1)*(NP-SCode.nTemp)+1:idx*(NP-SCode.nTemp)];
            P1 = Temp(Range, :);   %%%4591*100
            Temp = [SCode.P1; zeros([NP-SCode.nTemp,SCode.nTemp])];
            DT = -4*(Gamma(:, Range2)*Temp(Range, :))' * SCode.W;   %%%62*100
            dc1 = 1 /NP * (dcc + 2 * D' * (D * Gamma) + (bsxfun(@times, sign(Gamma), ...
                SCode.lamda_dic)));
            dc2 = SCode.C * [DT', -4*P1'];  %% 100*4653
            dC =  dc1 + dc2;
            Gamma  = Gamma  - SCode.mult_dic * 1 / sqrt(i) * dC;
            toc;
            end
    ...
    case 'Grad'
        Gamma = SCode.Code;NP = size(Gamma, 2);ND = size(Gamma, 1);
        ...
        L = real(abs(vals)).^0.5 * vecs';
            for i = 1:SCode.iter_dic
            tic;
            VT = (sum((L*Gamma(:, Range1)).^2, 1))';
            P1 = SCode.P1*Gamma(:, Range1)'*SCode.W;
            Q1 = bsxfun(@plus, SCode.Q0, (SCode.Q1 * VT));
            Q1  = Q1(:);
            TT = sum(-4*P1 .* repmat(Gamma(:, Range2)', SCode.ncons, 1), 2);
            TT = bsxfun(@plus, Q1', sum((reshape(TT, [], SCode.ncons)), 1));
            [a,idx] = max([sum(TT, 1), 0], [], 2);
            OBJ1 = 1 /NP * (sum(sum((data - D*Gamma).^2)) + sum(sum((bsxfun(@times, abs(Gamma), ...
                SCode.lamda_dic))))) + SCode.C * a;
            ...
            OBJ = OBJ1;
            Temp = [P1; zeros([NP-SCode.nTemp,ND])];
            Range = [(idx-1)*(NP-SCode.nTemp)+1:idx*(NP-SCode.nTemp)];
            P1 = Temp(Range, :);
            Temp = [SCode.P1; zeros([NP-SCode.nTemp,SCode.nTemp])];
            Q1 = [SCode.Q1; zeros([1, SCode.nTemp])];
            DT = (SCode.W * bsxfun(@times, Gamma(:, Range1), 2*Q1(idx,:)))' ...
                -  4*(Gamma(:, Range2)*Temp(Range, :))' * SCode.W;
            dC =  1 /NP * (dcc + 2 * D' * (D * Gamma) + (bsxfun(@times, sign(Gamma), ...
                SCode.lamda_dic))) + SCode.C * [DT', - 4*P1'];
            Gamma  = Gamma  - SCode.mult_dic * 1 / sqrt(i) * dC;
            toc;
            end
        end
    ...
    case 'Own-L0'
        tic;
        Gamma = sparsecode(data(:, 1:SCode.nTemp),D,XtX,G,thresh, 'L0',lamda);
        VT = diag(Gamma'*SCode.W*Gamma);
        SCode.P1 = SCode.P1*Gamma'*SCode.W;
        SCode.Q1 = bsxfun(@plus, reshape(SCode.Q00, [], SCode.ncons),  ...
            (SCode.Q1 * VT)');
        SCode.Q1  =SCode.Q1(:);
        Code = SCode.Code;NP = size(Code, 2);
        data = data(:, 1+SCode.nTemp:NP+SCode.nTemp);
        SCode.iter_dic = 2;
        TT = SCode.Q1 - sum(SCode.P1 .* repmat(Code', SCode.ncons, 1), 2);
        TT = reshape(TT, [], SCode.ncons);
        [a,idx] = max(TT, [], 2);
        idx = sub2ind([NP, SCode.ncons], [1:NP]', idx);
        P1 = SCode.P1(idx, :);
        
        data = data + (P1 * SCode.C * 2 / NP * pinv(D))';
        Gamma(:, 1+SCode.nTemp:NP+SCode.nTemp) = sparsecode(data,...
            D,XtX,G,thresh, 'L0',SCode.lamda_dic);
        toc;
        Gamma = sparse(Gamma);
end
end
