function [X, fobj, XDic] = mlr_Temp(NNT, nfac, Nneg, Npos, Margins, A, KmeanLAMDA, MeanK, X,XX, XDic, Y, SampleW, templateUP, ...
    Ymatched, W, SAMPLES, FeatUp, TemplateNorm, UpSolver, innc, ...
    INNCpara, innerfea, cparaINNC, ITERkmean, UPMODEL, Tround) 
fobj = 0;
if ~templateUP
    return;
end
...
if length(INNCpara) > 10 && INNCpara{11}
    if UPMODEL == 1
        ...
        tmp = NNT - Tround*fix(NNT/ Tround);
        tmp(find(tmp==0)) = Tround;
        if tmp > INNCpara{11}
            TempUp = 1;
        end
    end
...
switch innc
    case 0
        if TempUp
            [X(:, TempID), Feat_GD, fobj] = (mykmeans1(Margins, C, KmeanLAMDA, X(:, TempID), X(:, SAMPLES), ...
