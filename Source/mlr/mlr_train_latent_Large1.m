function [W, Xi, Diagnostics] = mlr_train_latent_Large1(X, Y, datastr, numSign, CIndex, ...
...
%     rotate = [0, 10, -10];
    ...
    CP          = @cuttingPlaneFull_latent_Large;
...
%         varargin = varargin{1};
    ...
    Template = setdiff([1:length(Y)], SAMPLES);
...
%         %%%%1.getLatentSample
        ...
        while Diagnostics.num_calls_solver < MAXITER
            ...
            [PsiNew, Mnew, SO_time, hLatent, RLatent, X]     = CP(k, X, hLatent, RLatent, PsiLatent, ...
...
% % %             %%%%debug
%             X11 = rand(size(X));
%             [W1, Xi, Dsolver1]              = mlr_admm(C, X11, Margins, H, Q);
% %             dis = W - W1; max(abs(dis(:)))
%             save('W1', 'W1');
%             PsiR1 = PsiR;
%             save('PsiR1', 'PsiR1');
% % %             %%%
