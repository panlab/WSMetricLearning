        ...
        for tt = 1:length(classes)
            ...
            for ii = crange
                for kkt = 1:length(HLOSS)
                    ...
                    load(fullfile(modelresult, 'Model-Round10_1.mat'), 'Metric');
                    Temp{i} = Metric{4};showimage(Temp{i}, 8, 9, 32);
                    dname = 'INF';
                    print(gcf, '-djpeg', '-r0', [dir dname '.jpg']);saveas(gca,[dir dname '.fig']);
                    print(gcf, '-depsc2','-r0', [dir dname '.eps']);
                    TempW{i} = (Metric{2}*Metric{4}')';
                    showimage(TempW{i}, 8, 9, 32);
                    dname = 'INFW';
                    print(gcf, '-djpeg', '-r0', [dir dname '.jpg']);saveas(gca,[dir dname '.fig']);
                    print(gcf, '-depsc2','-r0', [dir dname '.eps']);
                    for lamb = Lambda
                    LOSS{i2} = {[2, 3, 20, 100, 1, 10, 10, 1, 1, 2, 0, 0, -100.12, 302, 1], [lamb 1], [1]};
                    ...
                    TempW{i} = (Metric{2}*Metric{4}')';
                    showimage(TempW{i}, 8, 9, 32);
                    dname = [num2str(lamb), 'W'];
                    print(gcf, '-djpeg', '-r0', [dir dname '.jpg']);saveas(gca,[dir dname '.fig']);
                    print(gcf, '-depsc2','-r0', [dir dname '.eps']);

                    LOSS{i2} = {[2, 3, 20, 100, 1, 10, 10, 1, 1, 2, 0, 0, -100.12, 302, 1, 1], [lamb 1], [1]};
                    i =i + 1;clc;close all; clc;cd 'K:\Tanmin\project\Signdetect\SignClassify';[tmp, resultt(i), ranktime(i), ~, modelresult] = TrainFun1(dataset, [str, {[-1]}, {[]}, {{'', modelresult}}, [1]], Config,{'sift'},'MLR', {'1',classes{tt}}, [{{ii, HLOSS{kkt}, 1, 1, 0, 1, 3, MLRBalance(mm), 0, 1}, [{kk, 1}]}, LOSS{i2}], 1, 0, {{VirTest, newdata}, imsize}, -1, [4,8,16], 100, 1,{multiview(mm), kkk},0,DOGfig,'',0,npyra,0,0.5,0,'',0,4,[],0,{-10, ntr},PCAM,0,0,20,winit,[1,1,1,0,1], {Ratio_1(mm),[],Wfea{Find(mm)},KFea{Find(mm)}, 'Reciprocal',0,1,1, 'NA', WNorm{Find(mm)},Norm{Find(mm)}, weightN{Find(mm)}},1,1,0,Apcompute,1,featnorm);
                    load(fullfile(modelresult, 'Model-Round10_1.mat'), 'Metric');
                    Temp{i} = Metric{4};showimage(Temp{i}, 8, 9, 32);
                    dname = num2str(lamb);
...
% % % % % % % dir = ['K:\Tanmin\Study\CVPR\cvpr\cvpr14\latex\fig\', str, '\'];
...
% % % % % % % i3 = min(TTmp(:));i4 = max(TTmp(:));m2 = i4 - i3;
% % % % % % % i1 = i1 - m1*0.001;i2 = i2+m1*0.001;
% % % % % % % i3 = i3 - m2*0.001;i4 = i4+m2*0.001;
% % % % % % % Fbar = TTmp(1);
% % % % % % % TTmp = reshape(TTmp(2:end), 4, []);
% % % % % % % TTS = {'KNN-W(Metric)', 'KNN(Metric)', 'KNN-W', 'KNN'};
% % % % % % % loc = 'Best';
...
% % % % % % % plot([min(KNNrange):max(KNNrange)], Fbar,tStyle(styleID).color,...
...
% % % % % % % text(i2-25, i4, ['Our:', num2str(savedot(Fbar, 2))], ...
% % % % % % %     'fontsize', sizetitle*0.8, 'Color', [1 0 0]);
