function TestData = GetPCAFeature1(PCAMethod, featsize, LocalPCA, Normfea, Normfea1, ts_fea, PCACOEFF, FeatConf)
if isempty(ts_fea)
    TestData = [];
    return;
end
...
if iscell(PCACOEFF{1})
    ...
    TestData = getPCAfeaT(featsize, LocalPCA, PCAMethod, ts_fea, PCACOEFF, ts_label);
end

TestData = GetSparseCoding(Normfea1{1}, TestData, Normfea1{2}, Normfea1{3},...
    Normfea1{4}, Normfea1{5}, Normfea1{6});

% if isempty(Normfea1{1})
    TestData = myNormlize(TestData, Normfea);
% end


function TestData = getPCAfeaT(featsize, LocalPCA, PCAMethod, ts_fea, PCACOEFF, ts_label)
if LocalPCA
...
switch PCAMethod
    ...
    case 'SAE-NN'
        test_y = zeros(size(ts_fea,1), PCACOEFF{1}.size(end));
        ind = sub2ind([size(ts_fea,1), PCACOEFF{1}.size(end)], [1:length(ts_label)]', ts_label);
        test_y(ind) = 1;
        [er, bad] = nntest(PCACOEFF{1}, ts_fea, test_y);
