function [fdatabase, database, dFea] = getllcfeature(img_dir, dataname, fea_dir,  ...
    i, setting, copyremove, database)
% -------------------------------------------------------------------------
% parameter setting
database1 = database;
...
for jj = 1:length(feattype)
    ...
    database = TFdatabase(database1, data_dir_mf{jj});
   
%     database1 = retr_database_dir(data_dir_mf{jj}, img_dir, Sel_img_dir, Trainid);
% 
    if isempty(database),
        error('Data directory error!');
    end

    fprintf('Compute cooebook based on %s features for LLC : %s\n', feattype{jj});
