    ...
    for kkk = MMulti
        ...
        if ~isempty(outdim) && outdim(1) > 0
         ...
         for jj = pcarange
             for tt = 1:length(classes); 
                 try
                     i = i + 1;clc;close all; clc;cd(PATH_F);[temp, resultt(i), ranktime(i), ] = GetRecogRate_31(dataset, str, Config,{'sift'},'lmnn', {'1', classes{tt}}, [kk, dim, 80, 1, 0.5, 0], 1, 0, {{VirTest, newdata}, imsize}, -1, [4,8,16], 100, 1,kkk,0,DOGfig,'',0,npyra,0,0.5,0,'',0,Voting,[],0,nntrain,PCAM,0,0,20,0,[1], {1},1,0,0,Apcompute,1,featnorm);
                     i = i + 1;clc;close all; clc;cd(PATH_F);[temp, resultt(i), ranktime(i), ] = GetRecogRate_31(dataset, str, Config,{'sift'},'lmnn', {'1', classes{tt}}, [kk, dim, 200, 1, 0.5, 0], 1, 0, {{VirTest, newdata}, imsize}, -1, [4,8,16], 100, 1,kkk,0,DOGfig,'',0,npyra,0,0.5,0,'',0,Voting,[],0,nntrain,PCAM,0,0,20,0,[1], {1},1,0,0,Apcompute,1,featnorm);
                     i = i + 1;clc;close all; clc;cd(PATH_F);[temp, resultt(i), ranktime(i), ] = TrainFun(dataset, str, Config,{'sift'},'lmnn', {'1', classes{tt}}, [kk, dim, 500, 1, 0.5, 0], 1, 0, {{VirTest, newdata}, imsize}, -1, [4,8,16], 100, 1,kkk,0,DOGfig,'',0,npyra,0,0.5,0,'',0,Voting,[],0,nntrain,PCAM,0,0,20,0,[1], {1},1,0,0,Apcompute,1,featnorm);
                     i = i + 1;clc;close all; clc;cd(PATH_F);[temp, resultt(i), ranktime(i), ] = GetRecogRate_31(dataset, str, Config,{'sift'},'lmnn', {'1', classes{tt}}, [kk, dim, 1000, 1, 0.5, 0], 1, 0, {{VirTest, newdata}, imsize}, -1, [4,8,16], 100, 1,kkk,0,DOGfig,'',0,npyra,0,0.5,0,'',0,Voting,[],0,nntrain,PCAM,0,0,20,0,[1], {1},1,0,0,Apcompute,1,featnorm);
                 catch
        ...
        for dim = outdim
         ...
         for jj = pcarange
             for tt = 1:length(classes); 
                 try
                     i = i + 1;clc;close all; clc;cd(PATH_F);[temp, resultt(i), ranktime(i), ] = TrainFun(dataset, str, Config,{'sift'},'gblmnn', {'1', classes{tt}}, [kk, dim, 500, 1, 0.5, 0, nt], 1, 0, {{VirTest, newdata}, imsize}, -1, [4,8,16], 100, 1,kkk,0,DOGfig,'',0,npyra,0,0.5,0,'',0,Voting,[],0,nntrain,PCAM,0,0,20,0,[1], {1},1,0,0,Apcompute,1,featnorm);
                 catch
...
%             crangeSVM = [10, 16];
        ...
        for jj = pcarange
            for ii = crange
                for tt = 1:length(classes);
                    i = i + 1;clc;close all; clc;cd(PATH_F);[temp, resultt(i), ranktime(i), ] = TrainFun(dataset, str, Config,{'sift'},'MultiSVM', {'1', classes{tt}}, [ii,1,0], 1, 0, {{VirTest, newdata}, imsize}, -1, [4,8,16], 100, 1,kkk,0,DOGfig,'',0,npyra,0,0.5,0,'',0,Voting,[],0,nntrain,PCAM,0,0,20,0,[1], {1},1,0,0,Apcompute,1,featnorm);
                end
            end
        end
        ...
        for jj = pcarange
            for ii = crange
                for tt = 1:length(classes);
                    i = i + 1;clc;close all; clc;cd(PATH_F);[temp, resultt(i), ranktime(i), ] = TrainFun(dataset, str, Config,{'sift'},'MultiSVM', {'1', classes{tt}}, [ii,1,2], 1, 0, {{VirTest, newdata}, imsize}, -1, [4,8,16], 100, 1,kkk,0,DOGfig,'',0,npyra,0,0.5,0,'',0,Voting,[],0,nntrain,PCAM,0,0,20,0,[1], {1},1,0,0,Apcompute,1,featnorm);
                end
            end
        end
        ...
        for tt = 1:length(classes)
            ...
            for ii = crange
                for kkt = 1:length(HLOSS)
                    ...
                    i =i + 1;clc;close all; clc;cd(PATH_F);[tmp, resultt(i), ranktime(i), ~, modelresult] = TrainFun1(dataset, [str, {[-1]}, {[]}, {''}, [1]], Config,{'sift'},'MLR', {'1',classes{tt}}, [{{ii, HLOSS{kkt}, 1, 1, 0, 1, 3, MLRBalance(mm), 0, 1}, [{kk, 1}]}, LOSS{i2}], 1, 0, {{VirTest, newdata}, imsize}, -1, [4,8,16], 100, 1,{multiview(mm), kkk},0,DOGfig,'',0,npyra,0,0.5,0,'',0,4,[],0,nntrain,PCAM,0,0,20,winit,[1,1,1,0,1], {Ratio_1(mm),[],Wfea{Find(mm)},KFea{Find(mm)}, 'Reciprocal',0,1,1, 'NA', WNorm{Find(mm)},Norm{Find(mm)}, weightN{Find(mm)}},1,1,0,Apcompute,1,featnorm);
%                     weighted Metric+ template
                    LOSS{i2} = {[2, 3, 20, 100, 1, 10, 10, 1, 1, 2, 0, 0, -100.12, 302, 1], [0.0001 1], [1]};
                    i =i + 1;clc;close all; clc;cd(PATH_F);[tmp, resultt(i), ranktime(i), ~, modelresult] = TrainFun1(dataset, [str, {[-1]}, {[]}, {{'', modelresult}}, [1]], Config,{'sift'},'MLR', {'1',classes{tt}}, [{{ii, HLOSS{kkt}, 1, 1, 0, 1, 3, MLRBalance(mm), 0, 1}, [{kk, 1}]}, LOSS{i2}], 1, 0, {{VirTest, newdata}, imsize}, -1, [4,8,16], 100, 1,{multiview(mm), kkk},0,DOGfig,'',0,npyra,0,0.5,0,'',0,4,[],0,nntrain,PCAM,0,0,20,winit,[1,1,1,0,1], {Ratio_1(mm),[],Wfea{Find(mm)},KFea{Find(mm)}, 'Reciprocal',0,1,1, 'NA', WNorm{Find(mm)},Norm{Find(mm)}, weightN{Find(mm)}},1,1,0,Apcompute,1,featnorm);
%                     weighted Metric+ template+iterative template
                    i =i + 1;clc;close all; clc;cd(PATH_F);[tmp, resultt(i), ranktime(i), ] = TrainFun1(dataset, [str, {[-1]}, {[]}, {modelresult}, [1]], Config,{'sift'},'MLR', {'1',classes{tt}}, [{{ii, HLOSS{kkt}, 1, 1, 0, 1, 3, MLRBalance(mm), 0, 1}, [{kk, 1}]}, LOSS{i2}], 1, 0, {{VirTest, newdata}, imsize}, -1, [4,8,16], 100, 1,{multiview(mm), kkk},0,DOGfig,'',0,npyra,0,0.5,0,'',0,4,[],0,nntrain,PCAM,0,0,20,winit,[1,3,1,0,1], {Ratio_1(mm),[],Wfea{Find(mm)},KFea{Find(mm)}, 'Reciprocal',0,1,1, 'NA', WNorm{Find(mm)},Norm{Find(mm)}, weightN{Find(mm)}},1,1,0,Apcompute,1,featnorm);
%                     weighted Metric+ template+iterative M+template
                    LOSS{i2} = {[2, 3, 20, 100, 1, 10, 10, 1, 1, 2, 0, 0, -100.12, 302], [0.0001 1], [1]};
                    i =i + 1;clc;close all; clc;cd(PATH_F);[tmp, resultt(i), ranktime(i), ] = TrainFun1(dataset, [str, {[-1]}, {[]}, {modelresult}, [1]], Config,{'sift'},'MLR', {'1',classes{tt}}, [{{ii, HLOSS{kkt}, 1, 1, 0, 1, 3, MLRBalance(mm), 0, 1}, [kk]}, LOSS{i2}], 1, 0, {{VirTest, newdata}, imsize}, -1, [4,8,16], 100, 1,{multiview(mm), kkk},0,DOGfig,'',0,npyra,0,0.5,0,'',0,4,[],0,nntrain,PCAM,0,0,20,winit,[1,3,1,0,1], {Ratio_1(mm),[],Wfea{Find(mm)},KFea{Find(mm)}, 'Reciprocal',0,1,1, 'NA', WNorm{Find(mm)},Norm{Find(mm)}, weightN{Find(mm)}},1,1,0,Apcompute,1,featnorm);
                end
            end
            end
        end
...
for ct = range
    ...
    for kkk = MMulti
        ...
        resultt = [];ranktime = [];i = 0;i = i + 1;clc;close all; clc;cd(PATH_F);[temp, resultt(i), ranktime(i), ] = TrainFun(dataset, str, Config,{'sift'},'lmnn', {'1', classes{tt}}, [kk, dim, np, 1, 0.5, 0], 1, 0, {{VirTest, newdata}, imsize}, -1, [4,8,16], 100, 1,kkk,0,DOGfig,'',0,npyra,0,0.5,0,'',0,Voting,[],0,nntrain,PCAM,0,0,20,0,[1], {1},1,0,0,Apcompute,1,featnorm);
        ...
        resultt = [];ranktime = [];i = 0;i = i + 1;clc;close all; clc;cd(PATH_F);[temp, resultt(i), ranktime(i), ] = TrainFun(dataset, str, Config,{'sift'},'gblmnn', {'1', classes{tt}}, [kk, dim, np, 1, 0.5, 0, nt], 1, 0, {{VirTest, newdata}, imsize}, -1, [4,8,16], 100, 1,kkk,0,DOGfig,'',0,npyra,0,0.5,0,'',0,Voting,[],0,nntrain,PCAM,0,0,20,0,[1], {1},1,0,0,Apcompute,1,featnorm);
        resultN = [resultN, resultt];ranktimeN  = [ranktimeN, ranktime];


        tt = 1;ii = CbestVV(length(ranktimeN)+1); jj = pcarange(1);
        resultt = [];ranktime = [];i = 0;i = i + 1;clc;close all; clc;cd(PATH_F);[temp, resultt(i), ranktime(i), ] = TrainFun(dataset, str, Config,{'sift'},'MultiSVM', {'1', classes{tt}}, [ii,1,0], 1, 0, {{VirTest, newdata}, imsize}, -1, [4,8,16], 100, 1,kkk,0,DOGfig,'',0,npyra,0,0.5,0,'',0,Voting,[],0,nntrain,PCAM,0,0,20,0,[1], {1},1,0,0,Apcompute,1,featnorm);
        resultN = [resultN, resultt];ranktimeN  = [ranktimeN, ranktime];
...
% %         i = 0;resultt = [];
% %         for tt = 1:length(classes);
%         i = i + 1;clc;close all; clc;cd(PATH_F);[tmp, resultt(i), ranktime(i), ] = TrainFun1(dataset, str, Config,{'sift'},'KNN', {'1', classes{tt}}, {}, 1, 0, {{VirTest, '_PIE'}, imsize}, -1, [4,8,16], 100, 1,MMulti(mm),0,DOGfig,'',0,npyra,0,0.5,0,'',0,2,[],0,nntrain,PCAM,0,0,20, 0, 1, {1}, 0, 0, 11, 1);
%         i = i + 1;clc;close all; clc;cd(PATH_F);[tmp, resultt(i), ranktime(i), ] = TrainFun1(dataset, str, Config,{'sift'},'INNC', {'1', classes{tt}}, 0.05, 1, 0, {{VirTest, '_PIE'}, imsize}, -1, [4,8,16], 100, 1,MMulti(mm),0,DOGfig,'',0,npyra,0,0.5,0,'',0,2,[],0,nntrain,PCAM,0,0,20, 0, 1, {1}, 0, 0, 11, 1);
%         end 
