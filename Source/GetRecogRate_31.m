...
% May, 2010.
% =========================================================================
function [CPCave, CRavg, Cranktime, PerThresh, modelresult, CAresult] = GetRecogRate_31(dataname, suffix, config_file, ...
...
setting.mult_dic = 1;
setting.iter_dic = 10e3;setting.Newcode = 0;
suffx = '';
...
%     idx = strfind(nameresult, 'NF'); %%%new
%     nameresult1 = [nameresult(1:idx-1), nameresult(idx+2:end)]; %%%org
%     
%     idx = strfind(setting.Modelresult, 'NF');
%     Modelresult1 = [setting.Modelresult(1:idx-1), setting.Modelresult(idx+2:end)];
%     
%     fn = dir(nameresult1);
%     if length(fn) > 2 && ~isempty(fn(end).name)
%         dirrm(setting.Modelresult)
%         dirrm(nameresult)
%     else
%         fn = dir(nameresult);
%         if length(fn) > 2 && ~isempty(fn(end).name)
%             if ~exist(Modelresult1)  
%                 mkdir(Modelresult1)   
%             end;
%             if ~exist(nameresult1)  
%                 mkdir(nameresult1)   
%             end;
%             copyfile(setting.Modelresult, Modelresult1);
%             copyfile(nameresult, nameresult1);
%             dirrm(setting.Modelresult)
%             dirrm(nameresult)
%         end
%     end
...
% % % % try
% % % %     if APcompute
% % % %         Rname =[nameresult 'acc' setting.resultsuffix1 '.mat'];
% % % %     else
% % % %         Rname =[nameresult 'acc' setting.resultsuffix '.mat'];
% % % %     end
% % % %     Rname = fullfile(pwd, Rname);
% % % %     dos(['del ' Rname])
% % % % end



if length(dir(fullfile('K:/Tanmin/project/Signdetect/SignClassify', ...
        nameresult))) == 2
    try
        copyfile(fullfile('D:/MinTan/project/Signdetect/SignClassify', nameresult),...
            fullfile('K:/Tanmin/project/Signdetect/SignClassify', nameresult));
    end
end
...
% % % % % if CRavg~=0
% % % % %     File1 = setting.Modelresult;
% % % % %     CopyFILEMY(fullfile('K:\Tanmin\project\Signdetect\SignClassify', File1), ...
% % % % %         fullfile('D:\MinTan\project\Signdetect\SignClassify', File1));
% % % % %     File1 = setting.QualityResult;
% % % % %     CopyFILEMY(fullfile('K:\Tanmin\project\Signdetect\SignClassify', File1), ...
% % % % %         fullfile('D:\MinTan\project\Signdetect\SignClassify', File1));
% % % % % end
