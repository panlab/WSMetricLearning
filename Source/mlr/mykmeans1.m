function [XDic, Feat_GD, fobj] = mykmeans1(Margins, C, KmeanLAMDA, X, XX, Y, npos, nfac, k,...
...
%         XDic11 = GetINITSVD();
    ...
    [XDic, Feat_GD] = getDic(XDic1, X(:, 1+nTemp:end), SCode, SW, Tnorm,...
        ~isempty(cparaINNC), ITERkmean, MW);
...
%         OBJ1 = 0* 1 /NP * sum(diag((bsxfun(@times, data, sqrt(SW')))'*...
%             MW*(D*Gamma)))+SCode.C * a;
%         dc1 = 1 /NP * (MW*dcc/2);
%         dc2 = SCode.C * [DT'];
%         dC =  (0* dc1 + dc2);
