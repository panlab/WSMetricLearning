function [dPsi, M, SO_time, yrank, Yconsnew, YLoss] = cuttingPlaneFull(nfac,...
    k, X, W, SampleW, Ypos, Yneg, batchSize, SAMPLES, ClassScores, innerfea, ts_fold_idx, cbatchSize, cts_fold_idx, c1, ts_Fold_label)
...
% %     clear 'W'
    ntrain = length(TempID);
    ntest = length(SAMPLES);
    D = setDistanceFullMKL_f(X, L, ...
        ntrain + (1:ntest), 1:ntrain, innerfea);
...
%         parfor i = 1:batchSize
            if i <= length(SAMPLES)
                ...
                SO_start        = tic();
                [yi, li, maxsocre]        = SO(j - length(TempID), D, Ypos{j}, Ynegative, k);
...
% %                 TS1(i,:)         = PSIALL(j, [Ypos{j}, Ynegative]', n, Ypos{j}, Ynegative) * SampleW(i); 
% %                 TS2(i,:)         = PSIALL(j, yi', n, Ypos{j}, Ynegative) * SampleW(i); 
% %                 SS(i)  = maxsocre - li;
%                 zz2(i,:) = double(Yconsnew(i,1:length(TempID))) / nfac(i);
%                 zz1(i,:) = TS(i,1:length(TempID));
                
%                 Ycons(i,:)   = yi';
%                SS(i) = double(Yconsnew(i,:)) * (-D(j, TempID))' / nfac(i) + li;
