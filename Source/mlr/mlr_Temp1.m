function [X, fobj, XDic] = mlr_Temp1(NNT, nfac, Nneg, Npos, Margins, A, KmeanLAMDA, MeanK, X,XX, XDic, Y, SampleW, templateUP, ...
...
switch innc
    case 0
        [X(:, TempID), fobj] = (mykmeans1(Margins, C, KmeanLAMDA, X(:, TempID), X(:, SAMPLES), ...
    ...
    case 1
        [X(:, TempID), fobj, a, aidx, Feat_GD, SCode_C] = (mykmeans1(Margins, C, KmeanLAMDA, X(:, TempID), X(:, SAMPLES), ...
            Y(SAMPLES), Npos, nfac, MeanK, Ymatched, W, SampleW(:), Ycons, Yloss, ...
            PsiClock, TempID, TemplateNorm, UpSolver, INNCpara, innerfea, cparaINNC, ITERkmean));
    case 2
...
if TempUp
    if FeatUp
        PsiR = ComputePsiR(SAMPLES, X, Ycons, nfac, Npos, Nneg, ...
            Ymatched, innerfea, a, aidx, Feat_GD, SCode_C, W, C);    
    end
end


% function [m, fobj] = mykmeans1(Margins, A, KmeanLAMDA, mc, X, Y, npos, nfac, k,...
...
function PsiR = ComputePsiR(SAMPLES, X, Ycons, nfac, Npos, Nneg, Index, ...
    innerfea, a, aidx, Feat_GD, SCode_C, W, C)
global CPGRADIENT;
...
for i = 1:length(Ycons)
    ...
    PsiR{i}   = CPGRADIENT(X, S, batchSize);
    asum = -(PsiR{i}+Feat_GD).*W;
    
    sum(sum(-(PsiR{i}+Feat_GD).*W))
    sum(sum(-(Feat_GD).*W))
    sum(sum(-(PsiR{i}).*W))
