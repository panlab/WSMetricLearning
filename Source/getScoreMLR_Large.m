...
%                
    ...
    if ~setting.Mclassfy   %%%%testing
        try
            ...
            [X, Y, numSign, CIndex, latentinfo, ts_imname, LX, LY, LL, LR, Lscore, Lmargin, SignBased] = GetSampleLatent(pcastr, 0, ts_fold_idx, ts_idx, tr_idx,...
...
try
    load([setting.TFstr, '_', str, pcastr, '_data.mat'], 'cfeat')
catch
%     for jj = 1:length(fdatabase{1}.path{1})
    ...
    save([setting.TFstr, '_', str, pcastr, '_data.mat'], 'cfeat')
end

if islatent
    try
        load([setting.TFstr, '_', str, pcastr, '_celldata.mat'], 'Y', 'X', 'TnumSign', 'latentinfo')            
    catch
        nrotate =  length(setting.rotate);TnumSign = zeros(1, max(setting.ts_idx));
        for ijj = 1:length(setting.ts_idx)
            jj = setting.ts_idx(ijj);
            feat = cfeat{(jj)};
        ...
        save([setting.TFstr, '_', str, pcastr, '_celldata.mat'], 'Y', 'X', 'TnumSign', 'latentinfo')            
    end
end
