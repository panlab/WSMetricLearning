function [dPsi, M, SO_time, hLatent, RLatent] = cuttingPlaneFull_Latent(k, X, hLatent, RLatent, ...
...
%         parfor i = 1:batchSize
            if i <= length(SAMPLES)
                ...
                D = cell(1, Nrotate);
                for tt = 1:Nrotate
                    D{tt} = DISTANCE(W, [Xtemplate(:,[Range(tt,1):Range(tt,2)]), cell2mat(X(j))]);
                end
...
%     %%%end debug
%     
    dPsi    = PsiLatent - CPGRADIENT(getLatent(numSign, X, hLatent), S, batchSize);
end
