function [Psi, X, hLatent, Rlatent, tord] = ComputeLatent_Large_f(k,...
    KNNlatent, numSign1, X, hLatent,...
    Rlatent, numSign, CIndex, W, datastr, cellbase, SampleW, Ypos, Yneg, ...
...
%   SO_time     = time spent in separation oracle
    ...
    for i = 1:size(W,3)
        ...
        clear 'W'
         X = getLatent(cellbase, X, hLatent);
        [d,n,m] = size(X);
...
% % % % % % % % % % % % % % % % % % %                 %%%%%%%
                ...
                TS(j-ntrainS,:)         = PSIALL(j, [Ypos{j}; Ynegative'] + plus(Rlatent{j}),  ...
                    n1, Ypos{j} + plus(Rlatent{j}), Ynegative + plus(Rlatent{j}))* SampleW(j-ntrain);
            end
