...
path_truth = fullfile('SVMModel_truth',dataset);
path_test = fullfile('SVMModel',dataset);
...
for i = 3:length(test_dir_1)
    ...
    if ~exist(path_test_1,'dir')
        ...
        test_dir_2 = dir([path_truth_1 '/*Model*.mat']);
...
%                 test = struct2cell(load(path_test_2));
                truth = importdata(path_truth_2);
                test = importdata(path_test_2);
    ...
    %                         disp(truth{k}');
                    ...
                    temp = test{k} - truth{k};
                    if any(temp)
                        disp(['Error in:' path_test_2 '--' num2str(k)]);
%                         disp(test{k}');
%                         disp(truth{k}');
                        disp(reshape(temp,1,numel(temp)));
                    end
