function [lTe2,dists]=knnclassifytree_test(L,xTr,lTr,xTe,lTe,KK,varargin)
...
assert(~isempty(xTr));
assert(~isempty(xTe));
assert(size(xTr,2)==length(lTr));
assert(size(xTe,2)==length(lTe));
...
[iTe,dists]=usemtreemexomp(xTe,xTr,pars.tree,Kn);
...
% 
% if(pars.train)
%  [iTr,dists]=usemtreemexomp(xTr,xTr,pars.tree,Kn+1);
%  iTr=iTr(2:end,:);
