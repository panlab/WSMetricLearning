...
if nargin < 6  cpara = []; end
if nargin < 7  rawsize = [90, 75];   end
if nargin < 21  SetNormFea = 0;  end
if nargin < 22  ReCnew = 0;  end
if nargin < 23  coverage = [];  end
...
if nargin < 8 normmerge = 0; end
if nargin < 9 multiview = 0;end

setting.Rconfidence = Rconfidence;

setting.DoG = 0;config_DOG = '';
...
if exist(fn)  %%%modify feature type
    ...
    setting.WTA = 1;setting.WTAwithraw = 0;
else
    setting.WTA = 0; setting.WTAwithraw = 0;
end
setting.Fpyramid = Fpyramid;setting.Ratio = Ratio;setting.RRatio = RRatio;
setting.fast = 1;
  
setting.latent = 0;config_latent = '';
setting.bestS = 0;
...
if exist(fn)  %%%modify feature type
    ...
    setting.platent = 0;setting.platent = platent;
else
...
if nargin < 11  %%%for split , if ==1, then use fixed training/testing split
    UseSplit = 0;
    Nfold = 5;
else
    if iscell(UseSplit)
