function [acc, ranktime] = SiftFlow_Match1(setting, img_dir, tr_idx, ts_idx, ...
...
if ts_num < mem_block,
    ...
    th = tic;
    [IDX, distance] = GetRank_SFLOW(Samplevoted, setting, tr_fea, ts_fea, knn);
...
%     curr_ts_fea = zeros(mem_block, ADFea);
    ...
    curr_time = zeros(num_block, 1);
        
    C = [];
    distance = [];
        
    for jj = 1:num_block,
        fprintf('block number:%d / %d \n', jj, num_block);
        ...
        try
            load(fullfile(nameresult, [num2str(jj) '_result.mat']), 'IDX', 'curr_distance', 'curr_time', 'jj')
        catch
            th = tic;
            [IDX, curr_distance] = GetRank_SFLOW(Samplevoted, setting, tr_fea, curr_ts_fea, knn);
            curr_time(jj) = toc(th);
            save(fullfile(nameresult, [num2str(jj) '_result.mat']), 'IDX', 'curr_distance', 'curr_time', 'jj')
        end
        
        ranktime = ranktime + curr_time(jj);
