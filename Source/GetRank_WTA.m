...
if nargin == 13 & latentKNN ~= 1
    ...
    if latentKNN~=size(B,1) %%
        IDX = GetRank_WTA(Samplevoted, NotRatio, BitCount, ...
            K, isWTA, WTAwithraw, Metric1, WTAindex, innerfea, B, X, latentKNN);
        RNotRatio = NotRatio;
        NotRatio(:) = 0;
        if iscell(IDX)
            for jj = 1:length(IDX)
    ...
    [IDX, confidence] = GetRank_WTA(Samplevoted, NotRatio, BitCount, ...
        K, isWTA, WTAwithraw, Metric2, WTAindex, innerfea, B, X, knn);
    return;
end
% % if setting.MetricL2
...
if ~isWTA
    [IDX, confidence] = GetRank(Samplevoted, NotRatio, Metric, innerfea, B, X, knn);
    return;
end
...
if WTAwithraw
    [IDX, confidence] = GetRank_WTA_RAW(Samplevoted, NotRatio,BitCount, ...
        WTAindex,Metric, innerfea, B, X, knn);
    return;
end
