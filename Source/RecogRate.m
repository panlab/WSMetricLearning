function Ravg = RecogRate(dataname, suffix, Nclass, method, C, knnpara, multiview,...
    NFold, Round, evaltype, isappend, NeedHoG, ShowExist)
%INPUT
...
if nargin < 3  Nclass = '52'; end   
if nargin < 4  method = 'MLR'; end         
if nargin < 5  C = '512'; end   
if nargin < 6  knnpara = '1'; end 
if nargin < 7  multiview = '0';end   
if nargin < 8  NFold = '3';end  
if nargin < 9  Round = '1';end     
if nargin < 10 evaltype = '0';end   
if nargin < 11 isappend = 0;end    
if nargin < 12 NeedHoG = 1;end 
if nargin < 13 ShowExist = 0;end 

Nclass = str2num(Nclass);C = str2num(C);knnpara = str2num(knnpara);multiview = str2num(multiview);
NFold = str2num(NFold); Round = str2num(Round);evaltype= str2num(evaltype);
...
try
    if setting.recompute
        loadx(setting.ShowExist, [nameresult 'TTM.mat']);load([nameresult 'TTM.mat'], 'PCave', 'Ravg', 'ranktime');
    else
        if setting.testonly
            if APcompute
                resname = [nameresult 'acc' setting.resultsuffix1];

            else
                resname = [nameresult 'acc' setting.resultsuffix];
    ...
    if setting.FirstRound
        if setting.Usesubset
            accFF = zeros(setting.Ngroup, setting.Nclass);
            ranktimeFF = zeros(setting.Ngroup, 1);
            for ii = 1:setting.Ngroup
                Mstr = ['Round' num2str(Nfold) '_' num2str(setting.FirstRound(1)) 'G' num2str(setting.Ngroup) '_' num2str(ii) '.mat'];
                if APcompute
                    resname = fullfile(nameresult,['Result' setting.resultsuffix1 '-', Mstr]);
                    loadx(setting.ShowExist, resname); load(resname, 'accF', 'ranktimeF'); 
                else
                    resname = fullfile(nameresult,['Result' setting.resultsuffix '-', Mstr]);
                    loadx(setting.ShowExist, resname);load(resname, 'accF', 'ranktimeF');
                end
                accFF(ii, :) = accF;   ranktimeFF(ii) = ranktimeF;
            end
            accF = mean(accFF, 1);ranktimeF = mean(ranktimeFF);
        else
            if setting.TestOneR
                try
                    resname = fullfile(setting.Modelresult, ['Metric_',num2str(setting.TestR),'Round' num2str(Nfold) '_' num2str(setting.FirstRound(1))...
                        '.mat']);
                    loadx(setting.ShowExist, resname);
                    Mexist = 1;
                catch
                    Mexist = 0;
                end
                if Mexist
                    if APcompute
                        resname = fullfile(nameresult,['Result' setting.resultsuffix1 '-' 'Round' num2str(Nfold) '_' num2str(setting.FirstRound(1))...
                            '.mat']);
                        loadx(setting.ShowExist, resname); load(resname, 'accF', 'ranktimeF'); 
                    else
                        resname =fullfile(nameresult,['Result' setting.resultsuffix '-' 'Round' num2str(Nfold) '_' num2str(setting.FirstRound(1))...
                            '.mat']);
                        loadx(setting.ShowExist, resname); load(resname, 'accF', 'ranktimeF'); 
                    end
        ...
        if APcompute
            RavgI = mean(accF);
        PCaveI = accF;ranktime = ranktimeF;
        else
            Ravg = mean(accF);
        ...
        if APcompute
            resname = [nameresult 'acc' setting.resultsuffix1];
            loadx(setting.ShowExist, resname);load(resname, 'PCaveI', 'RavgI', 'ranktime');
        else
            resname = [nameresult 'acc' setting.resultsuffix];
            loadx(setting.ShowExist, resname);load(resname, 'PCave', 'Ravg', 'ranktime');
        end
    end
        end
    if evaltype
...
%     if setting.testonly
%         a = input(['The results matrix exists in file:\n ' nameresult '%s, please check it.\n If you want to test again, please input 1, otherwise input 0:\n'], 's');
%         while ~strcmp(a, '1') && ~strcmp(a, '0')
%             a = input(['The results matrix exists in file:\n ' nameresult '%s, please check it.\n If you want to test again, please input 1, otherwise input 0:\n'], 's');
%         end
%         if strcmp(a, '1')
%             1 / rand([1,4]);
...
%     end
    ...
    [fdatabase, database, dfea, WTAfea,setting] = computeFeature(img_dir1, img_dir, dataname, ...
        mfea_dir, mfea_dir_WTA, setting, feattype, suffix, knnpara);
    ...
    for Kround = 1:TNfold
        ...
        for jjj = 1:setting.Ngroup
            ...
            [tmp, C, rranktime(jjj,:)] = GetRoundAccuracy(evaltype, curround, Nfold, setting,img_dir1, dfea, ...
                WTAfea, fdatabase, feattype,knnpara, cpara, cmethod, nameresult, ...
                nameimresult,normmerge, multiview, latentresult1);
            if ~setting.testonly
            if ~isnan(tmp(end)) && tmp(end) == -inf
                imAP = 1;
                raccII(jjj) = tmp(end-1);
                rracc(jjj,:) = tmp(1:end-2);
            else
                rracc(jjj,:) = tmp;
            end
            end
        ...
        if ~setting.testonly
            ...
            racc(Kround,:) = mean(rracc, 1);
            raccI(Kround) = mean(raccII);
        end
    end
    if ~setting.testonly
        acc = mean(racc, 1);ranktime = mean(rranktime);
        PCave = acc;Ravg = mean(acc);
        fprintf('===============================================');

        fprintf('Average classification accuracy: %f\n', Ravg);
        fprintf('===============================================');
        if evaltype && ~Error && setting.SaveRes
            save([nameimresult 'issave'], 'issave');
        end
        PCaveI = 0;RavgI = mean(raccI);
    else
        RavgI = C;PCaveI = 0;ranktime= rranktime;
        save([nameresult 'acc' setting.resultsuffix '_I'], 'RavgI', 'ranktime');
    end
    
    if ~Error && setting.SaveRes
        if imAP
            resname = [nameresult 'acc' setting.resultsuffix '_I'];
            save(resname, 'PCaveI', 'RavgI', 'ranktime');
        end
        resname = [nameresult 'acc' setting.resultsuffix];
        save(resname, 'PCave', 'Ravg', 'ranktime');
    end
    if imAP
...
if ~setting.testonly
    ...
    case -1
       ...
       fprintf('The trained model is saved in :\n%s\n', setting.Modelresult)
       if setting.SnippetRatio{1} ~= 1
           fprintf('The trained reliablity model is saved in :\n%s\n', setting.QualityResult)
       end
    case 0
       fprintf('Testing done\n')
       fprintf('The used model is saved in :\n%s\n', setting.Modelresult)
       if setting.SnippetRatio{1} ~= 1
           fprintf('The used reliablity model is saved in :\n%s\n', setting.QualityResult)
       end
    end
    fprintf(['The Avearged acuuracy is ' num2str(Ravg) '\n'])
    
    [a,b] = fileparts(resname);fn  = fullfile(a, [b, '.txt']);
    fprintf('The results are saved in :\n%s\n', fn);
    if isappend
        fid = fopen(fn, 'a');
    else
        fid = fopen(fn, 'w');
    ...
    if APcompute
        fprintf(fid, 'The average accuracy:%f\r\n', Ravg);
    else
        fprintf(fid, 'Total average accuracy:%f\r\n\r\n', Ravg);
        fprintf(fid, 'The per-class average:\r\n');
        fprintf(fid, 'ClassID\tAccuracy\r\n');
        for tt  = 1:length(PCave)
            fprintf(fid, '%d\t%f\r\n', tt, PCave(tt));
        end
    end
    fclose(fid);
    ...
    if isempty(Ravg)
        fprintf('Error, results are not saved \n')
    else
        [a,b] = fileparts(resname);fn  = fullfile(a, [b, '.txt']);
        fprintf('The results are saved in :\n%s\n', fn);
        ...
        fprintf(fid, 'The predicted label:\r\n');
        for tt  = 1:size(Ravg{1}, 1)
            for jj = 1:size(Ravg{1}, 2)
                fprintf(fid, '%d\t', Ravg{1}(tt, jj));
            end
            fprintf(fid, '\r\n');
        end
        fprintf(fid, '\r\nSign filename:\r\n');
        for tt  = 1:length(Ravg{2})
            fprintf(fid, '%s\r\n', Ravg{2}{tt});
        end
        fprintf(fid, '\r\nFilename for the KNN templates:\r\n');
        for tt  = 1:size(Ravg{3}, 1)
            for jj = 1:size(Ravg{3}, 2)
                fprintf(fid, '%s\t', Ravg{3}{tt, jj});
            end
            fprintf(fid, '\r\n');
        end
        fclose(fid);  
