...
% % %         i = i + 1;clc;close all; clc;cd 'D:\MinTan\project\Signdetect\SignClassify';[tmp, resultt(i), ranktime(i), ] = TrainFun1(dataset, str, Config,{'sift'},'KNN', {'1', classes{tt}}, {}, 1, 0, {{VirTest, '_PIE'}, imsize}, -1, [4,8,16], 100, 1,MMulti(mm),0,DOGfig,'',0,npyra,0,0.5,0,'',0,2,[],0,{-10, ntr},PCAM,0,0,20, 0, 1, {1}, 0, 0, 11, 1);
% % %         i = i + 1;clc;close all; clc;cd 'D:\MinTan\project\Signdetect\SignClassify';[tmp, resultt(i), ranktime(i), ] = TrainFun1(dataset, str, Config,{'sift'},'INNC', {'1', classes{tt}}, 0.05, 1, 0, {{VirTest, '_PIE'}, imsize}, -1, [4,8,16], 100, 1,MMulti(mm),0,DOGfig,'',0,npyra,0,0.5,0,'',0,2,[],0,{-10, ntr},PCAM,0,0,20, 0, 1, {1}, 0, 0, 11, 1);
% % %         end 
         ...
         for jj = pcarange
             for tt = 1:length(classes); 
                 try
                     ...
                     for KNN = KNNrange
%                          i = i + 1;clc;close all; clc;cd 'D:\MinTan\project\Signdetect\SignClassify';[temp, resultt(i), ranktime(i), ] = TrainFun(dataset, str, Config,{'sift'},'lmnn', {['-' num2str(KNN)],classes{tt}}, [kk, dim, 500, 1, 0.5, 0], 1, 0, {{VirTest, newdata}, imsize}, -1, [4,8,16], 100, 1,kkk, 0,DOGfig,'',0,npyra,0,0.5,0,'',0,Voting,[],0,{-10, ntr},PCAM,0,0,20,0,[1], {1},1,0,0,Apcompute,1,featnorm);
                         i = i + 1;clc;close all; clc;cd 'D:\MinTan\project\Signdetect\SignClassify';[temp, resultt(i), ranktime(i), ] = TrainFun(dataset, str, Config,{'sift'},'lmnn', {[num2str(KNN)],classes{tt}}, [kk, dim, 500, 1, 0.5, 0], 1, 0, {{VirTest, newdata}, imsize}, -1, [4,8,16], 100, 1,kkk, 0,DOGfig,'',0,npyra,0,0.5,0,'',0,Voting,[],0,{-10, ntr},PCAM,0,0,20,0,[1], {1},1,0,0,Apcompute,1,featnorm);
                     end
                 catch
                     cd 'D:\MinTan\project\Signdetect\SignClassify';
                     resultt(end+1:end+length(KNNrange)) = 0; 
                     ranktime(end+1:end+length(KNNrange)) = 0; 
                 end
             end
         end
         end
...
% % % % % % %         try
% % % % % % %             resultt = (reshape(resultt, [length(classes)* length(KNNrange), length(outdim) * length(kkrange) * length(ntree)]))';
...
% % % % %         end
% % % % %         dir = 'D:\MinTan\Study\CVPR\cvpr\cvpr14\latex\fig\_PIE\Templates\';
% % % % % % % %         for tt = 1:length(classes)
% % % % % % % %             for kk = krange
% % % % % % % %             for ii = crange
...
% % % % % % % %                     i =i + 1;clc;close all; clc;cd 'D:\MinTan\project\Signdetect\SignClassify';[tmp, resultt(i), ranktime(i), ~, modelresult] = TrainFun1(dataset, [str, {[-1]}, {[]}, {''}, [1]], Config,{'sift'},'MLR', {'1',classes{tt}}, [{{ii, HLOSS{kkt}, 1, 1, 0, 1, 3, MLRBalance(mm), 0, 1}, [{kk, 1}]}, LOSS{i2}], 1, 0, {{VirTest, newdata}, imsize}, -1, [4,8,16], 100, 1,{multiview(mm), kkk},0,DOGfig,'',0,npyra,0,0.5,0,'',0,4,[],0,{-10, ntr},PCAM,0,0,20,winit,[1,1,1,0,1], {1,[],Wfea{Find(mm)},KFea{Find(mm)}, 'Reciprocal',0,1,1, 'NA', WNorm{Find(mm)},Norm{Find(mm)}, weightN{Find(mm)}},1,1,0,Apcompute,1,featnorm);
% % % % % % % % % % %                     %WM+te
% % % % % % % % % % %                     LOSS{i2} = {[2, 3, 20, 100, 1, 10, 10, 1, 1, 2, 0, 0, -100.12, 302], [0.0001 1], [1]};
% % % % % % % % % % %                     i =i + 1;clc;close all; clc;cd 'D:\MinTan\project\Signdetect\SignClassify';[tmp, resultt(i), ranktime(i), ] = TrainFun1(dataset, [str, {[-1]}, {[]}, {modelresult}, [1]], Config,{'sift'},'MLR', {'1',classes{tt}}, [{{ii, HLOSS{kkt}, 1, 1, 0, 1, 3, MLRBalance(mm), 0, 1}, [kk]}, LOSS{i2}], 1, 0, {{VirTest, newdata}, imsize}, -1, [4,8,16], 100, 1,{multiview(mm), kkk},0,DOGfig,'',0,npyra,0,0.5,0,'',0,4,[],0,{-10, ntr},PCAM,0,0,20,winit,[1,1,1,0,1], {1,[],Wfea{Find(mm)},KFea{Find(mm)}, 'Reciprocal',0,1,1, 'NA', WNorm{Find(mm)},Norm{Find(mm)}, weightN{Find(mm)}},1,1,0,Apcompute,1,featnorm);
% % % % % % % % % % %                  
% % % % % % % %                     %WM
% % % % % % % %                     LOSS{i2} = {[2, 3, 20, 100, 1, 10, 10, 1, 3, 2, 0, 0, 200, 100, 1], [0.000001 1], [1]};
% % % % % % % %                     i =i + 1;clc;close all; clc;cd 'D:\MinTan\project\Signdetect\SignClassify';[tmp, resultt(i), ranktime(i), ~, modelresult] = TrainFun1(dataset, [str, {[-1]}, {[]}, {''}, [1]], Config,{'sift'},'MLR', {'1',classes{tt}}, [{{ii, HLOSS{kkt}, 1, 1, 0, 1, 3, MLRBalance(mm), 0, 1}, [{kk, 1}]}, LOSS{i2}], 1, 0, {{VirTest, newdata}, imsize}, -1, [4,8,16], 100, 1,{multiview(mm), kkk},0,DOGfig,'',0,npyra,0,0.5,0,'',0,4,[],0,{-10, ntr},PCAM,0,0,20,winit,[1,1,1,0,1], {Ratio_1(mm),[],Wfea{Find(mm)},KFea{Find(mm)}, 'Reciprocal',0,1,1, 'NA', WNorm{Find(mm)},Norm{Find(mm)}, weightN{Find(mm)}},1,1,0,Apcompute,1,featnorm);
% % % % % % % %                     %WM+te
% % % % % % % %                     LOSS{i2} = {[2, 3, 20, 100, 1, 10, 10, 1, 1, 2, 0, 0, -100.12, 302, 1], [0.0001 1], [1]};
% % % % % % % %                     i =i + 1;clc;close all; clc;cd 'D:\MinTan\project\Signdetect\SignClassify';[tmp, resultt(i), ranktime(i), ~, modelresult] = TrainFun1(dataset, [str, {[-1]}, {[]}, {{'', modelresult}}, [1]], Config,{'sift'},'MLR', {'1',classes{tt}}, [{{ii, HLOSS{kkt}, 1, 1, 0, 1, 3, MLRBalance(mm), 0, 1}, [{kk, 1}]}, LOSS{i2}], 1, 0, {{VirTest, newdata}, imsize}, -1, [4,8,16], 100, 1,{multiview(mm), kkk},0,DOGfig,'',0,npyra,0,0.5,0,'',0,4,[],0,{-10, ntr},PCAM,0,0,20,winit,[1,3,1,0,1], {Ratio_1(mm),[],Wfea{Find(mm)},KFea{Find(mm)}, 'Reciprocal',0,1,1, 'NA', WNorm{Find(mm)},Norm{Find(mm)}, weightN{Find(mm)}},1,1,0,Apcompute,1,featnorm);
% % % % % % % %                     % weighted Metric+ template+iterative template
% % % % % % % %                     LOSS{i2} = {[2, 3, 20, 100, 1, 10, 10, 1, 1, 2, 0, 0, -100.12, 302], [0.0001 1], [1]};
% % % % % % % %                     i =i + 1;clc;close all; clc;cd 'D:\MinTan\project\Signdetect\SignClassify';[tmp, resultt(i), ranktime(i), ] = TrainFun1(dataset, [str, {[-1]}, {[]}, {modelresult}, [1]], Config,{'sift'},'MLR', {'1',classes{tt}}, [{{ii, HLOSS{kkt}, 1, 1, 0, 1, 3, MLRBalance(mm), 0, 1}, [kk]}, LOSS{i2}], 1, 0, {{VirTest, newdata}, imsize}, -1, [4,8,16], 100, 1,{multiview(mm), kkk},0,DOGfig,'',0,npyra,0,0.5,0,'',0,4,[],0,{-10, ntr},PCAM,0,0,20,winit,[1,3,1,0,1], {Ratio_1(mm),[],Wfea{Find(mm)},KFea{Find(mm)}, 'Reciprocal',0,1,1, 'NA', WNorm{Find(mm)},Norm{Find(mm)}, weightN{Find(mm)}},1,1,0,Apcompute,1,featnorm);
% % % % % % % %                  
% % % % % % % %                 end
% % % % % % % %             end
% % % % % % % %             end
...
% % % % % % % %         
% % % % % % % % % %         
% % % % % % % % % %         resultt = resultt([1, 2:2:end]);
% % % % % % % % % % %         resultN = resultt;
% % % % % % % % % %         if ~isempty(crangeMLR)
% % % % % % % % % %             resultN = [resultN, max(resultt(1:length(Lambda)+1:end))];
% % % % % % % % % %             ikk = 1;
% % % % % % % % % %             for lamb = Lambda
% % % % % % % % % %                 ikk = ikk + 1;
% % % % % % % % % %                 resultN = [resultN, max(resultt(ikk:length(Lambda)+1:end))];
% % % % % % % % % %             end
% % % % % % % % % %             [~, ord] = max(resultN(2:end));
% % % % % % % % % %             Lambda_best = Lambda(ord);
% % % % % % % % % %         end 
% % % % % % % % % %         
% % % % %         resultN = rand(1, 6);
% % % % %         Lambda_best = 0.0001;
...
% % % % % end
% % % % % 
% % % % % dir = ['D:\MinTan\Study\CVPR\cvpr\cvpr14\latex\fig\', str, '\'];
...
% % % % % % % %     save(['resultBTmpNNTemp1_N_Lamda', str, '.mat'], 'resultBTmp');
% % % % % % % % end
% % % % % % % % if strcmp(func2str(TrainFun), 'GetRecogRate_3')
% % % % % % % %     save(['resultBTmpNNTemp_N_Lamda', str, '.mat'], 'resultBTmp');
% % % % % % % % end
% % % % % close all;
% % % % % load(['resultBTmpNNTemp_N_Lamda', str, '.mat'], 'resultBTmp');
