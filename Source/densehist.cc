...
mxArray *process(const mxArray *mximage, const mxArray *mxsz, const mxArray *mxstride, 
  ...
  out[0] = max(blocks[0]-bound[0], 0);
  out[1] = max(blocks[1]-bound[1], 0);
  out[2] = nbin;
  ...
  int *off_dis = new int[nbin];
  int npixels = dims_sz[1]*dims_sz[0];
  double norm;

  for (int f = 0; f < nbin; f++) 
	  off_dis[f] = f*out[0]*out[1];
  ...
  for (int x = 0; x < dim[1]; ) { 
	  for (int y = 0; y < dim[0]; ) {
		  ...
		  case 2:
			  ...
			  norm = sqrt(norm + eps*eps);
			  for (int f = 0; f < nbin; f++) 
				  dst[off_dis[f]] = max(hist[f] / norm,0.2);
			  break;
		  case 3:
			  for (int f = 0; f < nbin; f++) 
				  norm += abs(hist[f]);
		  ...
		  case 4:
			  ...
			  norm = norm + eps;
			  for (int f = 0; f < nbin; f++) 
				  dst[off_dis[f]] = sqrt(hist[f] / norm);
			  break;
		  default:
